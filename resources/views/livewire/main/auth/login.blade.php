			...
			<div class="bg-white dark:bg-zinc-800 sm:max-w-md sm:w-full sm:mx-auto sm:rounded-lg sm:overflow-hidden shadow-sm border dark:border-zinc-700">
				<div class="px-4 py-8 sm:px-10">
					<div>
						...
						<p class="text-xl font-black text-gray-700 dark:text-gray-100 text-center mb-6">
						...
						@if ($social_grid)
							<div class="mt-1 grid grid-cols-{{ $social_grid }} gap-3">
								...
								@if (settings('auth')->is_facebook_login)
									<div>
										<a href="{{ url('auth/facebook') }}" class="w-full inline-flex justify-center py-3 px-5 rounded-sm bg-[#f7f7f7] hover:bg-[#eeeded] dark:bg-zinc-700 dark:hover:bg-zinc-600 text-sm font-medium">
								...
								@if (settings('auth')->is_google_login)
									<div>
										<a href="{{ url('auth/google') }}" class="w-full inline-flex justify-center py-3 px-5 rounded-sm bg-[#f7f7f7] hover:bg-[#eeeded] dark:bg-zinc-700 dark:hover:bg-zinc-600 text-sm font-medium">
								...
								@if (settings('auth')->is_github_login)
									<div>
										<a href="{{ url('auth/github') }}" class="w-full inline-flex justify-center py-3 px-5 rounded-sm bg-[#f7f7f7] hover:bg-[#eeeded] dark:bg-zinc-700 dark:hover:bg-zinc-600 text-sm font-medium">
								...
								@if (settings('auth')->is_twitter_login)
									<div>
										<a href="{{ url('auth/twitter') }}" class="w-full inline-flex justify-center py-3 px-5 rounded-sm bg-[#f7f7f7] hover:bg-[#eeeded] dark:bg-zinc-700 dark:hover:bg-zinc-600 text-sm font-medium">
								...
								@if (settings('auth')->is_linkedin_login)
									<div>
										<a href="{{ url('auth/linkedin') }}" class="w-full inline-flex justify-center py-3 px-5 rounded-sm bg-[#f7f7f7] hover:bg-[#eeeded] dark:bg-zinc-700 dark:hover:bg-zinc-600 text-sm font-medium">
					...
					<div class="mt-6">
						<div class="grid grid-cols-12 md:gap-x-6 gap-y-6">
							...
							<div class="col-span-6 flex items-center">
								<div class="relative flex items-start">
									<div class="flex items-center h-5">
									  <input id="remember_me" wire:model.defer="remember_me" type="checkbox" class="focus:ring-primary-600 h-4 w-4 text-primary-600 border-2 dark:bg-zinc-700 border-gray-200 dark:border-zinc-700 rounded-sm cursor-pointer">
									</div>
									<div class="ltr:ml-3 rtl:mr-3 text-sm">
									  <label for="remember_me" class="font-medium text-gray-700 dark:text-gray-400 cursor-pointer">{{ __('messages.t_remember_me') }}</label>
									</div>
								</div>
							</div>
							...
							<div class="col-span-6 ltr:text-right rtl:text-left">
								<a href="{{ url('auth/password/reset') }}" class="font-medium text-gray-700 dark:text-gray-400 text-sm">{{ __('messages.t_forgot_password') }}</a>
							</div>
				...
				{{-- Footer --}}
				<div class="px-4 py-6 bg-gray-50 dark:bg-zinc-700 border-t-2 border-gray-200 dark:border-zinc-700 sm:px-10 text-center">
		...
		<div class="pt-16 px-4 sm:px-6 sm:text-center md:max-w-2xl md:mx-auto lg:col-span-6 lg:text-left lg:flex lg:items-start">
			<div>
				...
				<div class="text-center md:rtl:text-right md:ltr:text-left w-full min-h-[150px]" wire:ignore>
					{{-- Text 1 --}}
					<h1 id="auth-text-1" class="text-3xl tracking-wide font-extrabold pb-0 mb-6 text-black dark:text-white leading-10 lg:text-4xl xl:text-5xl">
						{!! __('messages.t_register_slide_text_1', ['app_name' => config('app.name')]) !!}
					</h1>

					{{-- Text 2 --}}
					<h1 id="auth-text-2" class="text-3xl tracking-wide font-extrabold pb-0 mb-6 text-black dark:text-white leading-10 lg:text-4xl xl:text-5xl hidden">
				...
				{{-- Register button --}}
				<a href="{{ url('auth/register') }}" class="mt-10 block text-center w-full rounded-md border border-transparent px-5 py-3 bg-primary-600 text-base font-medium text-white shadow hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-600 focus:ring-offset-2 sm:px-10">{{ __('messages.t_signup') }}</a>
