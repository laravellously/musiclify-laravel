...
class CartComponent extends Component
{
    ...
    public function remove($id)
    {
        ...
        $this->dispatchBrowserEvent('alert',[
            "message" => __('messages.t_item_removed_from_cart_success')
        ]);
    }
    ...
    public function checkout()
    {
        ...
        foreach ($cart as $key => $item) {
            ...
            if ($gig) {
                ...
                if (is_array($item['upgrades']) && count($item['upgrades'])) {
                    ...
                    foreach ($item['upgrades'] as $index => $upgrade_id) {
                        ...
                        if ($upgrade) {
                            ...
                            // Show error message
                            $this->dispatchBrowserEvent('alert',[
                                "message" => __('messages.t_an_upgrade_in_item_in_cart_not_found'),
                                "type"    => "error"
                            ]);
                ...
                // Show error message
                $this->dispatchBrowserEvent('alert',[
                    "message" => __('messages.t_an_item_in_your_cart_doesnot_exist'),
                    "type"    => "error"
                ]);
