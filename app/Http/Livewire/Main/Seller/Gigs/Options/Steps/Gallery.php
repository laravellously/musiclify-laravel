...
class Gallery extends Component
{
    ...
    public function addVideo()
    {
        ...
        if (isset($matches[1])) {
            ...
            $headers = get_headers('https://www.youtube.com/oembed?format=json&url=http://www.youtube.com/watch?v=' . $matches[1]);
                ...
                if(is_array($headers) ? preg_match('/^HTTP\\/\\d+\\.\\d+\\s+2\\d\\d\\s+.*$/',$headers[0]) : false){
                    ...
                    $this->dispatchBrowserEvent('alert',[
                        "message" => __('messages.t_youtube_video_has_been_successfully_added'),
                    ]);
                    ...
                    // Error
                    $this->dispatchBrowserEvent('alert',[
                        "message" => __('messages.t_youtube_video_not_exists'),
                        "type"    => "error"
                    ]);
            ...
            // Error
            $this->dispatchBrowserEvent('alert',[
                "message" => __('messages.t_youtube_video_invalid'),
                "type"    => "error"
            ]);
    ...
    public function removeImage($id)
    {
        ...
        // Success
        $this->dispatchBrowserEvent('alert',[
            "message" => __('messages.t_file_has_been_successfully_deleted')
        ]);
    }
    ...
    public function save()
    {
        try {
            ...
            // Update gig
            $this->gig->video_id        = $this->video_id;
